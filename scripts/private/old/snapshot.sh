...
do
  ...
  if [[ $i = 0 ]]; then
    ...
    rm -r $SNAP_DIR/step-$i/app/assets/img
    rm -r $SNAP_DIR/step-$i/app/lib
    rm -rf $SNAP_DIR/step-$i/app/data/phones
  fi

  if [[ $i = 5 ]]; then
    cp -r app/assets/img $SNAP_DIR/step-0/app/
    cp -r app/data/phones $SNAP_DIR/step-0/app/
  fi
done
